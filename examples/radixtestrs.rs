...
use radixsort::{radix8sort_u32, radix8sort_u64, radix8sort_f32, radix11sort_u32, radix11sort_u64,
                radix11sort_f32};
use rand::{Rng, Rand};
use std::iter;
use std::vec::Vec;
...
fn test_radix<
    T: Clone + PartialOrd + FromPrimitive + Rand + PrintArrayElem,
...
>(
    ...
    let mut keys_temp: Vec<T> = iter::repeat(FromPrimitive::from_usize(0).unwrap())
        .take(size)
        .collect();
